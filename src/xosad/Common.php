...
class Common
{
	 ...
	 * @return string|null
	 */
	public static function request(string $url, string $params, bool $post = true, ?string $extra_params_string = '', string $proxy = ''): ?string
	{
		...
		if (!empty($params))
		{
			if ($post === true)
			{
				self::$curl_options[CURLOPT_POSTFIELDS] = $params . ($extra_params_string ? '&' . $extra_params_string : '');
			}
			else
			{
